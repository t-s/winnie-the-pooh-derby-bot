...
public class Bot {
	...
	public static void main(String[] args) throws AWTException,
		...
		while (!coordsSet) {
			// this loop will run forever, regardless of whether or not mouse
			// has been clicked
		...
		while (hits < PITCHES) {
			...
			outsideloop: for (x = 0; x < screenRect.width; x = x + 1) {
				for (y = 0; y < screenRect.height; y = y + 1) {
					...
					if (((color & 0xFF) > WHITE)
						...
						// right.
						if (!once) {
							robot.mouseMove(x + TOP_LEFT_X, TOP_LEFT_Y
									+ BASELINE_Y);
							if (!ballvisible)
								ballvisible = true;
							i++;
					...
					} else if ((closeTo((color & 0xFF), 255))
							&& (closeTo((color >> 8 & 0xFF), 90))
							&& (closeTo((color >> 16 & 0xFF), 25))) {
						once = false;
						robot.mouseRelease(InputEvent.BUTTON1_MASK);
						System.out.println("Strike!");
						robot.mouseMove(WIDTH_OF_AREA / 2 + TOP_LEFT_X,
								TOP_LEFT_Y + BASELINE_Y);
						allThreadSleep();
						hits++;

						break outsideloop;
					} else if ((closeTo((color & 0xFF), 83))
							&& (closeTo((color >> 8 & 0xFF), 130))
						...
						break outsideloop;
					} else if ((closeTo((color & 0xFF), 57))
							&& (closeTo((color >> 8 & 0xFF), 175))
							&& (closeTo((color >> 16 & 0xFF), 255))) {
						once = false;
						robot.mouseRelease(InputEvent.BUTTON1_MASK);
					...
					// end screen
					else if ((closeTo((color & 0xFF), 107))
							&& (closeTo((color >> 8 & 0xFF), 213))
